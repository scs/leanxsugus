...
Msg const *MainState_CaptureColor(MainState *me, Msg *msg)
{
	...
	case ENTRY_EVT:
		...
		data.pCurRawImg = data.frameBuffer;
		return 0;
	case FRAMESEQ_EVT:
...
OSC_ERR StateControl()
{
	...
	camErr = OscCamSetupCapture(0, OSC_CAM_TRIGGER_MODE_MANUAL);
	if (camErr != SUCCESS)
	{
	...
	{	/*----------- wait for captured picture */
		while(TRUE)
		{
			...
			camErr = OscCamReadPicture(0, (void * *) &pCurRawImg, 0, 0);
			if (camErr != -ETIMEOUT)
			{
		...
		else if (camErr != SUCCESS)
		{
			...
			OscLog(ERROR, "%s: Unable to read picture from cam. (%d)\n", __func__, camErr);
			break;
		}
