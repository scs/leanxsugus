...
Msg const *MainState_top(MainState *me, Msg *msg)
{
	...
	case IPC_SET_CAPTURE_MODE_EVT:
		/* If the IPC event is not handled in the actual substate,
		 * a negative acknowledge is returned by default. */
		data.ipc.enReqState = REQ_STATE_NACK_PENDING;
		return 0;
	}
	return msg;
...
Msg const *MainState_CaptureColor(MainState *me, Msg *msg)
{
	...
	//	memcpy(data.ipc.req.pAddr, data.u8ResultImageGray, sizeof(data.u8ResultImageColor))
	...
	case IPC_SET_CAPTURE_MODE_EVT:
		...
		{
			/* Need to capture raw images from now on, this is done
			 * in the captureRaw state.  */
...
Msg const *MainState_CaptureRaw(MainState *me, Msg *msg)
{
	...
	case IPC_GET_RAW_IMG_EVT:
		...
		memcpy(data.ipc.req.pAddr,
			data.pCurRawImg,
			OSC_CAM_MAX_IMAGE_WIDTH*OSC_CAM_MAX_IMAGE_HEIGHT);
			
		data.ipc.state.bNewImageReady = FALSE;
		
		/* Mark the request as executed, so it will be acknowledged
		 * later. */
		data.ipc.enReqState = REQ_STATE_ACK_PENDING;
		return 0;
	case IPC_SET_CAPTURE_MODE_EVT:
		...
		{
			/* Need to capture colored images from now on, this is done
			 * in the captureRaw state.  */
...
void MainStateConstruct(MainState *me)
{
	...
	StateCtor(&me->captureRaw, "Capture Raw",
			&((Hsm *)me)->top, (EvtHndlr)MainState_CaptureRaw);
	StateCtor(&me->captureColor, "Capture Color",
			&((Hsm *)me)->top, (EvtHndlr)MainState_CaptureColor);
}
...
OSC_ERR StateControl( void)
{
	...
	while( TRUE)
	{
		...
		while(TRUE)
		{
			...
			camErr = OscCamReadPicture(OSC_CAM_MULTI_BUFFER,
					(void**)&pCurRawImg,
					0,
					CAMERA_TIMEOUT);
			if(camErr != -ETIMEOUT)
		...
		data.pCurRawImg = pCurRawImg;
		 ...
		 /*----------- process frame by state engine (pre-setup)
		 * Sequentially with next capture */
	   ...
	   /*----------- process frame by state engine (post-setup)
		 * Parallel with next capture */
