...
static OSC_ERR QueryApp()
{
    ...
    if(err != SUCCESS)
    {
        /* This request is defined in all states, and thus must
         * succeed. */
...
static OSC_ERR SetOptions()
{
    ...
    {
    	err = OscIpcSetParam(cgi.ipcChan,
    			&pArgs->bDoCaptureColor,
    			SET_CAPTURE_MODE,
    			sizeof(pArgs->bDoCaptureColor));
    	if(err != SUCCESS)
    	{
...
int main()
{
	 ...
	 err = OscIpcRegisterChannel(&cgi.ipcChan,
