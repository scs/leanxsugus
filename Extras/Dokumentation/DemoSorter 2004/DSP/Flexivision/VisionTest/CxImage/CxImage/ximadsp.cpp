	...
	if (head.biClrUsed==0){
		...
		for(long y=ymin; y<ymax; y++){
			...
			for(long x=xmin; x<xmax; x++){
				...
				{
					color = GetPixelColor(x,y);
					color.rgbRed = (BYTE)max(0,min(255,(int)((color.rgbRed-128)*c + 128 + brightness)));
					color.rgbGreen = (BYTE)max(0,min(255,(int)((color.rgbGreen-128)*c + 128 + brightness)));
					color.rgbBlue = (BYTE)max(0,min(255,(int)((color.rgbBlue-128)*c + 128 + brightness)));
